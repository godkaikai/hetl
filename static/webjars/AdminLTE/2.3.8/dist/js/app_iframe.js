...
var App = function () {
    ...
    var basePath = '/webjars/AdminLTE/2.3.8/dist/';
    ...
    var handleIframeContent = function () {
        ...
        var height = App.getViewPort().height - $footer.outerHeight() - $header.outerHeight() - 5;
        if ($tabs.is(":visible")) {
    ...
    var handlePortletTools = function () {
        ...
        $('body').on('click', '.portlet > .portlet-title > .tools > a.reload', function (e) {
            ...
            //----------点击导航菜单跳转URL时自动拼接UID防止被安全拦截----------start
            //----------卢燕辉----------2019.7.17----------v0.1
            var __uid, separator;
            var reg = new RegExp('(^|&)__uid=([^&]*)(&|$)');
            ...
            if (__uid) {
            	...
            	url = url + separator + '__uid=' + __uid;
            }
          //----------点击导航菜单跳转URL时自动拼接UID防止被安全拦截----------end
        ...
        function init($menu_ul, data, level) {
            $.each(data, function (i, item) {
                ...
                if (item.children && item.children.length > 0) {
                    ...
                    var pullIcon = $('<i class="fa fa-caret-left pull-right"></i>');
                    ...
                    if (item.targetType != null && item.targetType === "blank") //代表打开新页面
                    {
                        ...
                        var href = 'addTabs({id:\'' + item.id + '\',title: \'' + item.text + '\',close: true,url: compileUrl(\'' + item.url + '\'),urlType: \'' + item.urlType + '\'});';
                        $a.attr('onclick', href);
                    }
